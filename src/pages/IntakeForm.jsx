...
const STATIC_QUESTIONS = [
    ...
    },
    {
        ...
        fields: [
            ...
            {
                name: "shoulder",
                label: "عرض الأكتاف (سم)",
                placeholder: "من الكتف للكتف",
            },
            {
                name: "sleeve",
                label: "طول الذراع (سم)",
                placeholder: "من الكتف للرسغ",
            },
        ],
    },
    {
        id: 'avatarCustomization',
        title: 'مواصفات العارضة (المودل)',
        subtitle: 'لنعرض التصميم على قوام ولون بشرة يشبهك تماماً لزيادة واقعية التصور',
        type: 'options',
        multiSelect: false,
        options: [
            'قوام ممتلئ (Curvy) - بشرة حنطية/سمراء',
            'قوام ممتلئ (Curvy) - بشرة فاتحة',
            'قوام نحيف/متوسط (Slim) - بشرة حنطية',
            'قوام نحيف/متوسط (Slim) - بشرة فاتحة',
            'متحجبة - قوام متوسط/فضفاض (Modest)',
        ],
    },
    {
        id: "clothingType",
        ...
        type: "options",
        // تم حصرها لتطابق الأقسام الحقيقية في الملف
        options: [
            "فستان سهرة / زفاف (Gown/Dress)",
            "عباية راقية (Abaya)",
            "بدلة نسائية رسمية (Suit/Tuxedo)",
            "طقم قطعتين (Two-Piece Set)",
        ],
    },
    {
        id: "occasion",
    ...
    },
    {
        ...
        subtitle: "تفاصيل الجزء العلوي للإطلالة (اختيار واحد إجباري)",
        type: "options",
        // تم الفصل التام للمصطلحات لمنع التضارب
        options: [
            "فتحة V عميقة",
            'ياقة قلب "Sweetheart"',
            "ياقة مربعة",
            'ياقة عالية دائرية "High Neck"',
            'ياقة بليزر رسمية "Lapel"',
            "ياقة أوف شولدر مكشوفة",
        ],
    },
    {
        id: "sleeves",
        ...
        type: "options",
        // تم دمج الطول والقصة في خيارات واضحة لتسهيل البحث على الوكيل
        options: [
            "أكمام طويلة ضيقة",
            "أكمام واسعة كلوش",
            'أكمام منفوخة "Puff"',
            "أكمام الكاب المتدلية",
            "بدون أكمام (علاقات أو مكشوف)",
        ],
    },
    {
        id: "fabricMaterial",
        ...
        type: "options",
        multiSelect: false, // ⚠️ إغلاق التحديد المتعدد
        options: [
            "ساتان ملكي",
            "تفتا واقفه",
            "مخمل",
    ...
    },
    {
        ...
        multiSelect: false, // ⚠️ إغلاق التحديد المتعدد
        options: [
            "شك خرز وكريستال يدوي",
            "دانتيل فرنسي",
            "ريش طبيعي",
            "نقشة جكار مدمجة",
            "تصميم نظيف كلاسيك بدون إضافات",
        ],
    },
    {
        id: "colors",
        title: "تدرجـات الألـوان (Colors)",
        subtitle: "المجموعة اللونية المطابقة للمنتجات",
        type: "options",
        multiSelect: false, // ⚠️ إغلاق التحديد المتعدد لمنع تشتت النموذج
        // تم حصر الألوان تماماً بما يتوافق مع مخزون المنتجات
        options: [
            "أسود",
            "كحلي (نيلي)",
            "عنابي",
    ...
    },
    {
        ...
        subtitle: "ملاحظة: في المسار اليدوي، الأولوية القصوى لخياراتك السابقة.",
        type: "textarea",
        placeholder:
            "مثال: أريد فستان مستوحى من حقبة الخمسينات ولكن بلمسة عصرية...",
    },
];
...
const IntakeForm = () => {
    ...
    const handleSelect = (option) => {
        if (currentQ.multiSelect) {
            const currentSelections = Array.isArray(answers[currentQ.id])
                ? answers[currentQ.id]
                : [];
    ...
    const handleNext = () => {
        if (
            currentQ.id === "preciseMeasurements" &&
            !answers["preciseMeasurements"]
        ) {
        ...
        if (
            ...
            answers["activeTrack"]?.includes("AI-Suggested")
        ) {
        ...
        if (nextStep < questions.length) {
            ...
            const payload = {
                ...
                // هذا السطر يخبر محرك الذكاء الاصطناعي بتجاهل مربع النص إذا كان يعارض الخيارات في المسار اليدوي
                strictMode:
                    answers["activeTrack"]?.includes("اليدوي") ||
                        answers["activeTrack"]?.includes("Manual")
                        ? true
                        : false,
            };

            console.log("👗 Final Payload Ready for Agent:", payload);
    ...
    return (
        <div className="min-h-screen flex items-center justify-center p-4 sm:p-6 bg-gradient-to-br from-[#fdf8f6] via-[#f2e8e5] to-[#eaddd7] overflow-hidden relative">
            ...
            <div className="glass-card max-w-2xl w-full p-8 shadow-2xl min-h-[500px] flex flex-col relative z-10 border border-white/50">
                ...
                <button
                    onClick={handleBack}
                    className="absolute top-8 right-8 text-primary-600 hover:text-primary-800 transition-colors"
                >
                    <ChevronRight className="w-6 h-6" />{" "}
                    {/* RTL: Right chevron is Back */}
                </button>
                ...
                <div className="flex-1 flex flex-col justify-center relative">
                    <AnimatePresence mode="wait" custom={direction}>
                        <motion.div
                            ...
                            className="w-full"
                        >
                            <h2 className="text-3xl font-bold font-arabic text-primary-900 mb-2">
                                {currentQ.title}
                            </h2>
                            <p className="text-gray-500 font-arabic mb-8 text-lg">
                                {currentQ.subtitle}
                            </p>
                            ...
                            {currentQ.type === "options" ? (
                                    ...
                                    {currentQ.id === "colors" &&
                                        ...
                                        answers.colors.includes(
                                            ...
                                            <motion.div
                                                ...
                                                className="p-5 bg-white/80 rounded-xl border border-primary-300 flex flex-col sm:flex-row items-center justify-between shadow-sm gap-4"
                                            >
                                                <span className="font-arabic text-primary-900 text-lg font-bold">
                                                    اختاري درجتك اللونية المفضلة بدقة:
                                                </span>
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    />
                                                    <span className="text-lg font-mono text-gray-700 bg-white px-3 py-1 rounded-md border shadow-inner uppercase">
                                                        {answers.customColorHex || "#d946ef"}
                                                    </span>
                                                </div>
                                            </motion.div>
                                        )}
                                ...
                                <div className="w-full h-full flex flex-col">
                                    ...
                                    <p className="text-sm font-arabic text-primary-600 mt-2 text-right">
                                        هذه الخطوة اختيارية، يمكنك الضغط على "التالي" لمعرفة اقتراح
                                        الذكاء الاصطناعي لكِ.
                                    </p>
                                </div>
                            ) : (
